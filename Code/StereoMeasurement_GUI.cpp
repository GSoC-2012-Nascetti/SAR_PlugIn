...
: QDialog(pParent)
{
   ...
   mpGetMapFileLocation = new QPushButton( "Get Map File Location", this );
   ...
   pLayout->addWidget( mpGetMapFileLocation, 6, 1 ); 
   ...
   ok = connect( mpGetMapFileLocation, SIGNAL( clicked() ),this, SLOT( GetMapFileLocation()));
...
void StereoMeasurement_GUI::GetMapFileLocation()
{    
	//APERTURA E LETTURA DEL FILE
	QString fileName = QFileDialog::getOpenFileName(this, tr("Select input image coordinate file"));

	QString fileOutput = QFileDialog::getSaveFileName(this, tr("Select output file"));
	...
	pProgress->updateProgress("Calculating result", 25, NORMAL);
	...
	FILE *file_in;
	file_in = fopen(fileName.toLocal8Bit().constData(),"r");

	if(file_in == NULL) return;

	double CoordI_Left=0., CoordJ_Left=0.,CoordI_Right=0., CoordJ_Right=0.;

	ofstream file_out;
	file_out.open(fileOutput.toLocal8Bit().constData());
	...
	pProgress->updateProgress("Calculating result", 50, NORMAL);
	while (! feof(file_in))
	{

		
		fscanf(file_in,"%lf %lf %lf %lf",&CoordI_Left,&CoordJ_Left,&CoordI_Right,&CoordJ_Right);
		// RETRIEVE 3D COORDINATE //
		COORD_Ecef Result = sModel.Stereo_SAR_SlantToGround(CoordI_Left,CoordJ_Left,CoordI_Right,CoordJ_Right);

		file_out <<Result.Latitude<<" "<<Result.Longitude<<" "<<Result.Height<<endl;
		
	}

	file_out.close();

	std::string msg = "3D restitution complete";	
